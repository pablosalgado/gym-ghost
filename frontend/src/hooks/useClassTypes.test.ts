...
describe('useClassTypes', () => {
  ...
  it('shows loading state while fetching', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ activities: MOCK_CLASS_TYPES }),
      })
    )
  ...
  it('filters class types by facilityId when provided', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ activities: [MOCK_CLASS_TYPES[0]] }),
      })
    )

    const { result } = renderHook(() => useClassTypes(5))

    await waitFor(() => expect(result.current.isLoading).toBe(false))

    expect(result.current.classTypes).toEqual([MOCK_CLASS_TYPES[0]])
    expect(fetch).toHaveBeenCalledWith('/api/v1/activities?facility_id=5', {
      headers: { Authorization: `Bearer ${AUTH_TOKEN}` },
    })
  })
  ...
  it('re-fetches when facilityId changes', async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ activities: [MOCK_CLASS_TYPES[1]] }),
    })
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      '/api/v1/activities?facility_id=5',
      {
        headers: { Authorization: `Bearer ${AUTH_TOKEN}` },
      }
    )
