...
- For a non-Docker static deployment, build in `frontend/`, then copy `frontend/dist/.` into `public/`. The production Docker image performs that handoff itself; `docker-compose.yml` runs it with a persistent SQLite volume and requires `APP_HOSTS`, `SECRET_KEY_BASE`, and `ATTR_ENCRYPTED_KEY` in a local `.env` file. Production assumes a TLS-terminating proxy that forwards the original HTTPS scheme (`X-Forwarded-Proto: https`) and host headers.
...
| Partner integration test (RSpec) | `spec/integration/partner/auth_service_spec.rb` | Excluded from `bundle exec rspec`; run via `bundle exec rspec --tag integration` |
